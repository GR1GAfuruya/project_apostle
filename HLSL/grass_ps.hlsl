...
{
    return 1;
    float4 base_color = texture_maps[0].Sample(sampler_states[ANISOTROPIC], pin.texcoord);
    return base_color;
}
#else
[earlydepthstencil]
float4 main(GS_OUT pin) : SV_TARGET
{
	...
	float3 L = 0;
    L = normalize(-light_direction[0].xyz);
    ...
    radiance += (diffuse + specular) * light_color.rgb * light_color.w;
