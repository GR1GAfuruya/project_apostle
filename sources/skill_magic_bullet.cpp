...
// 
//==============================================================
MagicBullet::MagicBullet(DirectX::XMFLOAT3* init_pos, DirectX::XMFLOAT3* dir, PublicParam init_param)
{
	initialize();
	...
	//エフェクト
	{
		main_effect = std::make_unique<MeshEffect>("./resources/Effects/Meshes/eff_spear.fbx");
	...
	{
		lightning_disk_effect = std::make_unique<MeshEffect>("./resources/Effects/Meshes/disk.fbx");
	...
	{
		l = std::make_unique<MeshEffect>("./resources/Effects/Meshes/lightning.fbx");
	...
	//ライト生成
	spear_light = make_shared<PointLight>(*launch_pos, 30.0f, DirectX::XMFLOAT3(1.0f, 0.8f, 5.5f));
...
// 
//==============================================================
void MagicBullet::update(float elapsed_time)
{
	//弾射出
	if (!launch_bullet)
	{
	...
	for (auto& l : lightning_effect)
	{
		l->update(elapsed_time);
	}

	lightning_disk_effect->update(elapsed_time);
...
// 
//==============================================================
void MagicBullet::render(Camera* camera)
{
	if (is_hit)
	{
	}
	main_effect->render(camera);

	for (auto& l : lightning_effect)
	{
		l->render(camera);
	}
	lightning_disk_effect->render(camera);
