...
void Trail::initialize(ID3D11Device* device, const wchar_t* FileName_, const wchar_t* ColorMapName_)
{
    HRESULT hr{ S_OK };
    ...
    hr = device->CreateBuffer(&bufferDesc, &subResourceData, &mVertexBuffer);
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

    if (FileName_)
    {
        hr = load_texture_from_file(device, FileName_, &mShaderResourceView, &mTexture2DDesc);
        _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
    }
    if (ColorMapName_)
    {
        hr = load_texture_from_file(device, ColorMapName_, &mTrailColorSrv, &mTexture2DDesc);
        _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
    }
    ...
    hr = create_vs_from_cso(device, cso_name, mVertexShader.GetAddressOf(), mInputLayout.GetAddressOf(),
        inputElementDesc, _countof(inputElementDesc));
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

    //ピクセルシェーダーオブジェクトの生成
    const char* cso_ps_name{ "./shaders/SwordTrailPs.cso" };
    hr = create_ps_from_cso(device, cso_ps_name, mPixelShader.GetAddressOf());
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

    // ジオメトリシェーダーを作成
    const char* cso_gs_name{ "./shaders/SwordTrailGs.cso" };
    hr = create_gs_from_cso(device, cso_gs_name, mGeometryShader.GetAddressOf());
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

    // 定数バッファを作成
    mConstantBuffer = make_unique<Constants<TrailConstantBuffer>>(device);
    mConstantBuffer->data.mThreshold = 1.0f;
}

void Trail::fUpdate(float elapsedTime_, size_t steps)
{
...
void Trail::fRender(ID3D11DeviceContext* pDeviceContext_)
{
    ...
    mConstantBuffer->bind(pDeviceContext_, 0,CB_FLAG::ALL);
...
void Trail::fAddTrailPoint(DirectX::XMFLOAT3 Top_, DirectX::XMFLOAT3 Bottom_)
{
...
void Trail::fEraseTrailPoint(float elapsedTime_)
{
    bool isErase = false;
...
void Trail::fInterpolate(size_t steps)
{
