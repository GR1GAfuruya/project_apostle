...
ObjectMesh::ObjectMesh(ID3D11Device* device, const wchar_t* obj_filename, bool flipping_v_coordinates)
{
	...
	create_vs_from_cso(device, "shaders/object_mesh_vs.cso", vertex_shader.GetAddressOf(),
		input_layout.GetAddressOf(), input_element_desc, ARRAYSIZE(input_element_desc));
	//ジオメトリシェーダーオブジェクトの作成
	create_gs_from_cso(device, "shaders/object_mesh_gs.cso", geometry_shader.GetAddressOf());
	//ピクセルシェーダーオブジェクトの作成
	create_ps_from_cso(device, "shaders/object_mesh_ps.cso", pixel_shader.GetAddressOf());
...
void ObjectMesh::create_com_buffers(ID3D11Device* device, vertex* vertices, size_t vertex_count, uint32_t* indices, size_t index_count)
{
	HRESULT hr{ S_OK };
...
void ObjectMesh::render(ID3D11DeviceContext* immediate_context, const DirectX::XMFLOAT4X4& world, const DirectX::XMFLOAT4& material_color,
	ID3D11PixelShader* bound_pixel_shader)
{
	...
	for (const material& material : materials)
	{
		...
		object_constants->bind(immediate_context, 0, CB_FLAG::PS_VS_GS);
		for (const subset& subset : subsets)
		{
