...
void DebugFigures::render_all_figures(ID3D11DeviceContext* context)
{
	...
	for (const Sphere& sphere : spheres)
	{
		...
		if (DebugFlag::get_wireframe_switching()) context->Draw(sphere_vertex_count, 0);
	}
	spheres.clear();
	// 円柱描画
	...
	for (const Cylinder& cylinder : cylinders)
	{
		...
		figure_constants->bind(context, 0, CB_FLAG::VS);
		if (DebugFlag::get_wireframe_switching()) context->Draw(cylinder_vertex_count, 0);
	}
	cylinders.clear();
	// 直方体描画
	...
	for (const Cuboid& cuboid : cuboids)
	{
		...
		if (DebugFlag::get_wireframe_switching()) context->DrawIndexed(buffer_desc.ByteWidth / sizeof(uint32_t), 0, 0);
	}
	cuboids.clear();
	// カプセル描画
	context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
	...
	for (const Capsule& capsule : capsules)
	{
		...
		figure_constants->bind(context, 0, CB_FLAG::VS);
		if (DebugFlag::get_wireframe_switching()) context->Draw(capsule_vertex_count, 0);
	}
	capsules.clear();
#endif // _DEBUG
}
