...
public:
    ...
    void bind(ID3D11DeviceContext* dc, UINT slot, CB_FLAG flags = CB_FLAG::PS)
    {
        ...
        if (flags & CB_FLAG::HS) dc->HSSetConstantBuffers(slot, 1, constant_buffer.GetAddressOf());
        if (flags & CB_FLAG::DS) dc->DSSetConstantBuffers(slot, 1, constant_buffer.GetAddressOf());
        if (flags & CB_FLAG::GS) dc->GSSetConstantBuffers(slot, 1, constant_buffer.GetAddressOf());
    }

    //外部でデータを作って代入したいときに使う
    void data_set(T add_data)
    {
        data = add_data;
    }
    //--------< 変数 >--------//
